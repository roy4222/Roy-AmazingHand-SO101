    ...
    for event in node:
        ...
        if event_type == "INPUT":
            ...
            if event_id == "tick":
                ...
                s1_pitch = np.sin(2.0 * np.pi * 1.0 * elapsed) * np.radians(
                    10.0
                ) + np.radians(10.0)
                rs1_roll = np.cos(2.0 * np.pi * 1.0 * elapsed) * np.radians(10.0)
                ls1_roll = np.cos(2.0 * np.pi * 1.0 * elapsed) * np.radians(-10.0)

                s2_pitch = np.sin(2.0 * np.pi * 1.0 * elapsed) * np.radians(
                    140.0 / 2.0
                ) + np.radians(140.0 / 2.0)
                s4_pitch = np.sin(2.0 * np.pi * 1.0 * elapsed) * np.radians(
                    (90.0 + 53.0) / 2.0
                ) + np.radians((90.0 - 53.0) / 2.0)
                ...
                rtip2 = Rotation.from_euler(
                    "XYZ", [np.radians(10.0), s2_pitch, 0.0]
                )  # finger2 has a 10° roll offset
                rtip2 = rtip2.as_quat(scalar_first=True)

                ltip2 = Rotation.from_euler(
                    "XYZ", [np.radians(-10.0), s2_pitch, 0.0]
                )  # finger2 has a 10° roll offset
                ...
                rtip3 = Rotation.from_euler(
                    "XYZ", [np.radians(20.0), s2_pitch, 0.0]
                )  # finger3 has a 20° roll offset
                rtip3 = rtip3.as_quat(scalar_first=True)

                ltip3 = Rotation.from_euler(
                    "XYZ", [np.radians(-20.0), s2_pitch, 0.0]
                )  # finger3 has a 20° roll offset
                ...
                rtip4 = Rotation.from_euler(
                    "xyz", [0.0, -s4_pitch, np.radians(20.0)]
                )  # finger4 has a 20° yaw offset
                rtip4 = rtip4.as_quat(scalar_first=True)

                ltip4 = Rotation.from_euler(
                    "xyz", [0.0, -s4_pitch, np.radians(-20.0)]
                )  # finger4 has a 20° yaw offset
